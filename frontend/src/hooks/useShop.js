...
const useShop = () => {
    ...
    const updateItem = async (itemId,formData) => {
        ...
        try {
            const response = await axiosInstance.post(`${import.meta.env.VITE_BA_URL}/api/item/edit-item/${itemId}`, formData)
            return response?.data
    ...
    const getItemById = async (itemId) => {
        try {
            const result = await axiosInstance.get(`${import.meta.env.VITE_BA_URL}/api/item/get-item-by-id/${itemId}`)
            setCurrentItem(result?.data.item);
        } catch (error) {
            return error?.response?.data || { error: true };
        }
    ...
    return { shop, items, loading, createShop, createItem, getItemById, currentItem, updateItem }
